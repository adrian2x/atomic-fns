...
export const lowOrderMatrix = {
    ...
    ...lowOrderMatrix
}, exactDaysInYear = 146097.0 / 400, exactDaysInMonth = 146097.0 / 4800, exactMatrix = {
    years: {
        quarters: 4,
        months: 12,
...
export class Duration {
    ...
    add(duration) {
        const dur = new Duration(duration), result = {};
...
function convert(matrix, sources, fromUnit, result, toUnit) {
    const conv = matrix[toUnit][fromUnit], raw = sources[fromUnit] / conv, sameSign = Math.sign(raw) === Math.sign(result[toUnit]), 
    // ok, so this is wild, but see the matrix in the tests
    added = !sameSign && result[toUnit] !== 0 && Math.abs(raw) <= 1 ? antiTrunc(raw) : Math.trunc(raw);
    result[toUnit] += added;
