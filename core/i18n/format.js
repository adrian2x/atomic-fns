const NATIVE_DATE = /^(\d{4})[-/]?(\d{1,2})?[-/]?(\d{0,2})[Tt\s]*(\d{1,2})?:?(\d{1,2})?:?(\d{1,2})?[.:]?(\d+)?$/;
/**
 ...
 */
export function getDate(date, utc = false) {
    if (date === null)
        return new Date(NaN);
    if (!date)
        return new Date();
    ...
    if (typeof date === 'string' && !/Z$/i.test(date)) {
        const d = date.match(NATIVE_DATE);
        if (d) {
            const m = d[2] - 1 || 0;
...
const n = 'numeric', s = 'short', l = 'long', dd = '2-digit', TOKEN_MAP = {
    // weekday
    '%A': { weekday: l },
    '%a': { weekday: s },
    '%aa': { weekday: n },
...
const formatter = (locale, opts) => new Intl.DateTimeFormat(locale, opts);
/**
 ...
 */
export function format(fmt, date, locale) {
    let results = [], dayPeriod = '', dayPeriodIndex = -1;
    for (let i = 0; i < fmt.length; i++) {
        ...
        if (part === '%') {
            ...
            while (i < fmt.length - 1 && fmt[i + 1] === fmt[i])
                part += fmt[++i];
            if (HOUR_PARTS[part]) {
    ...
    if (dayPeriodIndex >= 0)
        results[dayPeriodIndex] = dayPeriod || '';
    return results.join('');
}
